...
if [[ ${PV} == *9999 ]]; then
	...
	SRC_URI="
		https://github.com/Taiko2k/${PN}/archive/refs/tags/v${PV}.tar.gz -> ${P}.tar.gz
	"
	KEYWORDS="~amd64"
fi
...
DEPEND="
	...
	dev-python/isounidecode[${PYTHON_USEDEP}]
	media-video/ffmpeg
...
#cp -r ${WORKDIR}/miniaudio-$_miniaudiocommit/* src/phazor/miniaudio/
#cp -r ${WORKDIR}/kissfft-$_kissfftver/* src/phazor/kissfft/

src_prepare() {
	# Workaround. If PATCHES is used it would be applied to times and fail
	eapply "${FILESDIR}/${PN}-fix-desktop.patch"
	use openmp && eapply "${FILESDIR}/${PN}-fix-kissfft-openmp.patch"
...
python_install() {
        ...
        dolib.so  "${S}_build/libphazor.so"
	...
	# link libphazor to prevent error due to a hardcode check of library path
